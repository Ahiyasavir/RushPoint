...
has a test** — add a scenario, don't just add the callable. The table below lists **97** callables
...
| `admin/index.ts` | **listPlatformUsers** (admin-only creator activity rollup: games created, runs launched, derived last-active — see `apps/creator-web` `/admin/users`) |
