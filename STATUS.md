...
## ✅ הושלם — ממשקים מבוססי-תפקיד (לפי דרישות המשתמש)

מימוש 11 דרישות חדשות שהמשתמש ביקש — **כולן הושלמו**. סטטוס מעקב:

| # | דרישה | סטטוס |
|---|-------|-------|
| 10 | גודל קבוצה 4–7 (לקוח + שרת + e2e) | ✅ הושלם |
| 6 | שופט רואה שמות חברים + מספר + טלפון קפטן | ✅ הושלם |
| 3 | אסור לדלג על זיווג (הוסר bypass) | ✅ הושלם |
| 8 | מערכת תפקידים — בחירת תפקיד פשוטה (4 תפקידים) | ✅ הושלם (אומת בדפדפן) |
| 9 | הגבלת פיצ'רים רגישים למנהל אירוע בלבד | ✅ הושלם (דרך gating) |
| 1 | לשונית "קבוצות" מציגה את השלב הנוכחי | ✅ הושלם (אומת בדפדפן) |
| 2 | "קרא לאיש צוות" — חירום/תקלה + סאונד + מידע מלא | ✅ הושלם (UI אומת בדפדפן) |
| 5 | מתנדבת מאשרת הגעה לתור → עוצרת טיימר | ✅ הושלם (אומת בדפדפן) |
| 7 | קונסולת מתפעל תחנה (25 תחנות) | ✅ הושלם (אומת בדפדפן) |
| 11 | ממשק מתנדב (כל הקבוצות, קנסות, חירום, מידע) | ✅ הושלם (אומת בדפדפן) |
| 4 | אפשרות "לשופט" אחרי הטנא + לוגיקת 90 שניות | ✅ הושלם (קנס ספרינט אומת מול האמולטור) |

### מערכת תפקידים (#8) — `apps/admin/src/roles.tsx`
4 תפקידים: **מנהל אירוע** (גישה מלאה) · **שופט טנא** (`/judge` בלבד) · **מתפעל תחנה** (`/station`, בוחר מספר תחנה 1–25) · **מתנדב** (`/volunteer`).
מסך בחירת תפקיד (`RoleSelect`) מוצג כשאין תפקיד; הבחירה נשמרת ב-localStorage; ה-nav והראוטר מסננים לפי `ROLE_ROUTES`.
כפתור "החלפת תפקיד" בכותרת. הערה: gating בצד-לקוח בלבד (דמו) — לפרודקשן יש להחליף ב-custom claims.

### לוגיקת ספרינט לשופט (#4)
דדליין קשיח = `craftingStart + 20 דק׳ + 90 שנ׳` (יציאה מוקדמת = הזמן שנותר + 90, אותו דדליין מוחלט).
`finalizeJudgeEvaluation` מחיל `computeSprintPenalty(secondsLate)` (אקספוננציאלי, תקרה 300) על משבצת ה-gold,
לפי חותמת ה-check-in מול הדדליין. כפתור "ללכת לשופט" נוסף ליד תפריט מילוי הטנא ב-`basket-zone`.

> ⚠️ אומת בדפדפן: כל ממשקי האדמין (תפקידים, קבוצות+שלב, התראות, מתנדב, מתפעל תחנה). קנס הספרינט אומת מול האמולטור.
> **לא אומת ויזואלית:** מסכי המובייל החדשים (קריאה-לצוות, כפתור "ללכת לשופט") — עברו typecheck בלבד; דורש זרימת רישום→gold מלאה במובייל.
...
> ✅ **תוקן:** המובייל קורא עכשיו ל-`requestNextTask` אוטומטית כשמשבצת ירוק נפתחת בלי `taskId`;
> השרת כותב `taskId`/`taskTitle` בחזרה ל-`gameState`, וכפתור "הגעתי לשופט" חסום עד שמוקצית משימה אמיתית.
