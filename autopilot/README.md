# RushPoint Autopilot — 5-Day Autonomous Improvement System
...
```
 ...
 │
 ├─ create per-cycle branch  autopilot/cycle-N  (off the integration branch)
 │
...
```
autopilot/
├── README.md            ← this file (design + start/stop/resume)
├── config.json          ← model, duration, validation commands, limits  (deliverable #3)
├── supervisor.mjs       ← the supervisor loop                            (deliverable #2)
├── lib/
│   ├── state.mjs        ← state.json load/save (atomic) + recovery       (deliverable #4, #7)
│   ├── claude.mjs       ← runs `claude -p`, parses JSON, detects rate limits (deliverable #3)
│   ├── git.mjs          ← branch / commit / merge / discard helpers
│   ├── validate.mjs     ← runs typecheck / lint / e2e                    (validation)
│   └── files.mjs        ← regenerates the human-readable .md mirrors
├── prompts/
│   ├── selector.md      ← task-selection prompt  (deliverable #3, #5)
│   ├── implementer.md   ← coding prompt          (deliverable #3)
│   └── reviewer.md      ← review prompt           (deliverable #3, #6)
...
2. **Recovers** any interrupted cycle: re-queues the in-flight task, checks out the integration
   branch, and deletes dangling `autopilot/cycle-*` branches (integration is always clean).
...
- `git log autopilot/main` — the actual merged improvements.
...
- **Integration branch is sacred.** `autopilot/main` only moves forward via a reviewed + validated
