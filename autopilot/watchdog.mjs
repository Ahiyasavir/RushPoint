#!/usr/bin/env node
// watchdog.mjs — keeps the autopilot supervisor running FOREVER (until you say stop).
//
...
async function main() {
  ...
  if (existsSync(STOP_FLAG)) { log('STOP flag is set (autopilot was stopped by the user) — not starting. Run `node autopilot/supervisor.mjs start` to resume.'); process.exit(0); }
