...
- [x] **1. RED (pure):** new `scripts/test-import-sheet.ts` — `parseGameRows`: valid rows → stages/tasks;
  unknown type → error; quiz without answer → error; bad coords → error; empty → empty game.
  Run `npm test` → RED.
- [x] **2. GREEN:** add `parseGameRows` + `RowError` type to `packages/shared/src/` (reuse
  `validation.ts`), export. Re-run → green.
- [ ] **3. DEFERRED → frontend agent (creator-web Builder Import panel + lib/importSheet.ts CSV/xlsx reader):** Builder "Import" panel — template download; drag-drop → `lib/importSheet.ts`
