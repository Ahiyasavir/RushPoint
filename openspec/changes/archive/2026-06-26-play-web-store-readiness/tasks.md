...
- [x] **1. Write `scripts/test-manifest.ts` and confirm it FAILS for the right reason.**
...
- [x] **2. Redesign the source `icon.svg`** to the "Velocity Compass" mark (see design.md → *The
...
- [x] **3. Add the icon generator + `sharp` devDep.**
...
- [x] **7. Run `npx tsx scripts/test-manifest.ts` → it MUST now PASS.** Confirm via the aggregator
  that `node scripts/run-unit-tests.mjs` auto-discovers and runs it green.

## REFACTOR / scaffolding for the store step

- [x] **8. Add the `.well-known/assetlinks.json` placeholder** (`[]` + header comment that the real
  package name + signing SHA-256 are filled post-domain per `STORE.md`). Do not invent values.

- [x] **9. Write `apps/play-web/STORE.md`** — the runbook from design.md (domain → Firebase Hosting →
...
- [x] **10. Run the full gate set and confirm green:**
