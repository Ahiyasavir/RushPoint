...
- [x] **1. RED (pure):** new `scripts/test-free-mode.ts` — `resolveLaunchBilling` (payments off →
...
- [x] **2. GREEN:** add `PAYMENTS_ENABLED` (= false) + `resolveLaunchBilling` + `isFeatureUnlocked`
  + `FREE_MODE_MAX_PARTICIPANTS` to `packages/shared/src/`, export. Re-run → green.

## Backend wiring

- [x] **3. RED (e2e):** in `scripts/e2e-verify.mjs` (payments off) — a 0-credit free-plan creator
  launches a run successfully with no wallet decrement; `purchaseCredits`/`subscribePro` →
  `failed-precondition`. Run `npm run e2e` → RED.
- [x] **4. GREEN:** `launchRun` uses `resolveLaunchBilling` (skip the wallet transaction when
...
- [x] **5. GREEN (UI):** gate `WalletPage` route, pricing, "buy"/"upgrade" copy + CTAs (`i18n.ts`,
...
- [x] **6. REFACTOR:** confirm flipping `PAYMENTS_ENABLED = true` in a local build restores the wallet
  page, the launch billing, and the Pro gates (sanity check, then flip back to false).
- [x] **7. Full gate set:** `npm run typecheck` · `npm run lint` · `npm test` ·
