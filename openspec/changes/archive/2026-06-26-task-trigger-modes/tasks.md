...
- [x] 1.1 Create `scripts/test-trigger-modes.ts` importing `evaluateTrigger`, `defaultRadiusFor`,
...
- [x] 2.1 Add `export type TriggerMode = 'radius' | 'exact' | 'instant' | 'locationless'` and
  `triggerMode?: TriggerMode` to `Task` in `packages/shared/src/types/index.ts` (documented default
  `'radius'`).
- [x] 2.2 Add `defaultRadiusFor`, `evaluateTrigger`, `normalizeTriggerMode` to
  `packages/shared/src/geo.ts`; re-export from `packages/shared/src/index.ts`.
- [x] 2.3 Run `npm test`; confirm `test-trigger-modes.ts` passes (GREEN).

## 3. RED → GREEN — Server enforcement in completeTask

- [x] 3.1 Add failing assertions to `scripts/e2e-verify.mjs`: an `exact` task rejects a far check-in
...
- [x] 3.2 In `functions/src/runs/index.ts` `completeTask`, replace the `type==='geofence'` block with
...
- [x] 3.3 Confirm `sanitizeTaskForParticipant` exposes `triggerMode`+`geofenceRadiusMeters`
  (non-secret); no answer-key leak.
- [x] 3.4 Run `npm run e2e`; confirm all assertions pass (GREEN).

## 4. GREEN — Wizard Step 1 selector (edits task-creation-wizard)

- [x] 4.1 Add `TRIGGER_MODE_META` (4 entries: icon/label/description) to
...
- [x] 4.2 In `BuilderPage.tsx` TaskEditor Step 1, replace the binary locationless toggle with a
...
- [x] 5.2 `npm test` — trigger-modes test green.
- [x] 5.3 `npm run lint` and `npm run creator:build` — pass.
- [x] 5.4 `npm run e2e` — full lifecycle incl. exact + instant assertions green.
