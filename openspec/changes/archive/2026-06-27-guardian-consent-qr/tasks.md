...
- [x] **1. RED (pure):** new `scripts/test-guardian-consent.ts` — `isConsentSatisfied`: not required →
  true; required + no record → false; required + record → true. Run `npm test` → RED.
- [x] **2. GREEN:** add `isConsentSatisfied` + `ConsentRecord` + `Game.requiresGuardianConsent`/`minAge`
  to `packages/shared/src/`, export. Re-run → green.
- [x] **3. RED (e2e):** in `scripts/e2e-verify.mjs` — consent-required run: team cannot start until
  `grantGuardianConsent`; used/invalid token refused; non-consent run starts normally. `npm run e2e` → RED.
- [x] **4. GREEN:** implement `requestGuardianConsent` + `grantGuardianConsent` in
