...
- [x] 1.1 Create `scripts/test-registration-fields.ts` importing `resolveRegistrationFields` and
...
- [x] 1.2 Run `npm test`; confirm failure ("not exported"/"is not a function") (RED).

## 2. GREEN — Shared helpers

- [x] 2.1 Create `packages/shared/src/registration.ts` with `isNameField`,
...
- [x] 2.3 Run `npm test`; confirm `test-registration-fields.ts` passes (GREEN).

## 3. GREEN — Wire JoinScreen

- [x] 3.1 In `apps/play-web/src/screens/JoinScreen.tsx`, replace inline `teamFields`/`memberFields`/
...
- [x] 3.2 Compute `memberNames`/`displayName` via `resolveDisplayName`; keep the `joinRun` payload shape.

## 4. Verify

- [x] 4.1 `npm run typecheck` — 0 errors.
- [x] 4.2 `npm test` — registration test green.
- [x] 4.3 `npm run e2e` — join lifecycle still green (no callable change).
- [x] 4.4 Preview: solo game shows one name field; team game shows team-name + member list.
