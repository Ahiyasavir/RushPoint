...
- [ ] **1. RED (e2e):** in `scripts/e2e-verify.mjs`, join two teams (uid_A, uid_B); as uid_A call
...
- [ ] **6. GREEN:** add `requireStorageUrl` to `packages/shared/src/validation.ts`, export it; call it
...
- [ ] **7. RED (pure):** `scripts/test-staff-throttle.ts` — `shouldLockout(attempts, limit)` +
  `isWithinCooldown(...)` boundary cases. Run → fails (helpers absent).
- [ ] **8. GREEN:** implement the predicates (shared or functions util); in `staffSignIn` persist
...
- [ ] **9. RED (pure + e2e):** `scripts/test-attempt-limit.ts` boundary predicate; e2e: a quiz with
...
- [ ] **10. GREEN:** in `submitTaskAnswer`, read the task's `attemptLimit`; persist `taskAttempts[taskId]`
...
- [ ] **12. GREEN:** move the run + accessCode `set` into the billing `runTransaction`; `playCount`
...
- [ ] **14. GREEN:** add a per-referrer window/cap in `claimReferral`; require `proExpiresAt > now` in
  the `launchRun` Pro branch. Green.

## Gate — all green before done
- [ ] **15. Full gate set:** `npm run typecheck` · `npm run lint` · `npm test` (incl. new pure tests) ·
