...
- [ ] **1. RED (pure):** new `scripts/test-discovery-poi.ts` — assert:
...
- [ ] **2. GREEN:** add all four helpers + `DiscoveryPoi` / `DiscoveryPoiResult` /
...
- [ ] **3. RED (rules):** in `scripts/test-rules.mjs` assert: creator `get`/`list` of
  `discoveryPois` succeeds; play-client (different uid) `get`/`list` denied. Run
  `npm run test:rules` → fails RED.
- [ ] **4. GREEN:** add the `discoveryPois` rule block to `firestore.rules`. Re-run → green.
  Confirm existing e2e still passes.

## Callables

- [ ] **5. RED (e2e):** in `scripts/e2e-verify.mjs` add a discovery POI to the seeded game; after
...
- [ ] **6. GREEN:** implement `getRunDiscoveryPois` + `claimDiscoveryPoi` in
...
- [ ] **7. GREEN (UI):** new Builder "Discovery POIs" panel — list of POIs; add/edit/delete; map
...
- [ ] **8. GREEN (UI):** play-web background geofence watcher (using GPS from `updateLocation`
...
- [ ] **9. REFACTOR:** confirm idempotency in the overlay (overlay does not re-show for claimed POIs
  on GPS fluctuation); confirm answer normalisation edge cases (Hebrew text, numbers). Preview-verify.
- [ ] **10. Full gate set:** `npm run typecheck` · `npm run lint` · `npm test` ·
