...
- [ ] **1. RED (pure):** new `scripts/test-watermark.ts` (auto-discovered by `run-unit-tests.mjs`) —
...
- [ ] **2. GREEN:** add `resolveShareQrTarget` + `computeWatermarkLayout` to `packages/shared/src`,
  export from `packages/shared/src/index.ts`. Re-run → green.

## The watermark helper + QR

- [ ] **3.** Add `qrcode` to `apps/play-web/package.json` (+ `@types/qrcode` dev).
- [ ] **4.** New `apps/play-web/src/lib/brandWatermark.ts`: `loadImage(src)`, `makeQrImage(target)`
...
- [ ] **5. GREEN (UI):** refactor `apps/play-web/src/lib/storyCard.ts` to call `stampBrand` instead of
...
- [ ] **6. GREEN (UI):** new `apps/play-web/src/lib/sharePhoto.ts` — `sharePhoto(photoUrl, brandData)`
...
- [ ] **7. REFACTOR:** confirm the fallback paths — force `logoImg=null` (text wordmark renders) and
  simulate a tainted canvas (falls back, no throw). Preview-verify both.

## Gate — all green before done
- [ ] **8. Full gate set:** `npm run typecheck` · `npm run lint` · `npm test` (incl. `test-watermark`)
