...
- [x] **1. RED (pure):** new `scripts/test-run-recap.ts` (auto-discovered) — assert `buildRunRecap`
...
- [x] **2. GREEN:** add `buildRunRecap` + `computeMontageGrid` + recap result type + the `?recap=`
  route key to `packages/shared/src`, export from `index.ts`. Re-run → green.

## Callable — getRunRecap

- [x] **3. RED (e2e):** in `scripts/e2e-verify.mjs`, after finalize: owner `getRunRecap` → standings +
...
- [x] **4. GREEN:** implement `getRunRecap` in `functions/src/runs/index.ts` (resolve accessCode →
...
- [x] **5. GREEN (UI):** new `apps/play-web/src/screens/RunRecap.tsx` + `?recap=<accessCode>` route in
  `App.tsx`; render standings + photo montage. Verify via preview tools.
- [~] **6. PARTIAL:** play-web "Share recap" collage (computeMontageGrid + stampBrand) DONE in recapCollage.ts/RunRecap.tsx. Creator RunConsole "Share recap" action DEFERRED → frontend agent (creator-web). ORIG: "Share recap" builds the collage (`computeMontageGrid` + `stampBrand` from
...
- [x] **7. REFACTOR:** confirm retention path in UI (a pruned run shows standings, empty montage, no
  crash) and that the recap ordering matches the live leaderboard.

## Gate — all green before done
- [x] **8. Full gate set:** `npm run typecheck` · `npm run lint` · `npm test` (incl. `test-run-recap`)
