...
- [x] **1. RED (pure):** new `scripts/test-streak.ts` — assert `computeStreak` (consecutive
...
- [x] **2. GREEN:** add `computeStreak` + `computeMedianTaskMs` to `packages/shared/src/`, export
  from `index.ts`. Re-run → green.
- [x] **3. GREEN (UI):** `useStreak` hook in play-web; render streak chip in `PlayScreen.tsx` /
