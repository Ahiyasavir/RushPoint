...
- [x] **1. RED (pure):** new `scripts/test-run-analytics.ts` — assert `computeRunAnalytics`:
...
- [x] **2. GREEN:** add `computeRunAnalytics` + `RunAnalytics` type + `compareToPlatformMedian` stub
  to `packages/shared/src/`, export from `index.ts`. Re-run → green.

## Callable

- [x] **3. RED (e2e):** in `scripts/e2e-verify.mjs` after finalize: owner `getRunAnalytics` →
  expected per-task structure; non-owner → `permission-denied`. Run `npm run e2e` → fails RED.
- [x] **4. GREEN:** implement `getRunAnalytics` in `functions/src/runs/index.ts` (owner-only,
...
- [ ] **5. DEFERRED → frontend agent (creator-web RunConsole Analytics tab: Pro-gated heatmap + sortable table + benchmark arrows):** new Analytics tab in creator-web RunConsole — Pro gate (upsell chip for
...
- [x] **6.** `npm run typecheck` · `npm run lint` · `npm test` · `npm run creator:build` · `npm run e2e`.
