...
- [x] **1. RED (pure):** new `scripts/test-playtest-links.ts` — `resolveEmulatorHost` (default
...
- [x] **2. GREEN:** add the three helpers to `packages/shared/src/`, export. Re-run → green.

## Client emulator host

- [x] **3. GREEN:** `apps/*/src/services/firebase.ts` use `resolveEmulatorHost(import.meta.env,
...
- [x] **4. GREEN:** rewrite `scripts/proxy.mjs` route table via `resolveProxyTarget` (v2 creator/play +
  emulator paths; drop the v1 :8081 fallback).
- [x] **5. GREEN:** add `npm run playtest` (EMU + SEED + CREATOR + PLAY at 0.0.0.0 + proxy:3000 +
...
- [ ] **6. HUMAN manual verify (phone over cellular):** run `npm run playtest`; open the creator link on the laptop, the join link
  on a phone over cellular → the phone joins the seeded run and plays. Write `PLAYTEST.md` runbook.
- [x] **7. Gate:** `npm run typecheck` · `npm run lint` · `npm test` · `npm run creator:build`.
