...
and SHALL list no URL that does not correspond to a published page. The `robots.txt` of
that app SHALL NOT disallow any landing page path.

#### Scenario: Sitemap lists exactly the published pages

- **WHEN** the sitemap URL set is compared with the published landing page set
