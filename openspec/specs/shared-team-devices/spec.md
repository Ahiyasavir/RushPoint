# shared-team-devices Specification

## Purpose
Let several phones attach to one team and follow the game live, while exactly one device at a
time (the controller) may submit; control is transferable mid-game and can be taken over so the
team is never stuck behind a dead phone.
