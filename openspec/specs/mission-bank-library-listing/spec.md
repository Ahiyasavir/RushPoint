# mission-bank-library-listing Specification

## Purpose

The task library (the Builder's mission picker and the Gallery's task tab) lists
the hand-authored mission bank alongside published `publicTasks`, so the bank's
~89 missions are reachable without a Firestore seed or backfill and stay in sync
with the bank source and admin overrides automatically.
