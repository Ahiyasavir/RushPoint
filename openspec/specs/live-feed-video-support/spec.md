# live-feed-video-support Specification

## Purpose
TBD - created by archiving change run-media-gallery-and-video-feed. Update Purpose after archive.
## Requirements
...
Every UI that renders `feedItems` (the participant/staff live feed panel and the creator-web Run Console's feed panel) SHALL render an item whose `mediaKind` is `'video'` using a video player
