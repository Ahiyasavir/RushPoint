...
export default function CheckInsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('Could not load pending check-ins. Is the emulator running?');
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-bold text-white">Check-in Review</h1>
        <button onClick={() => void load()} className="text-sm text-zinc-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-zinc-800">Refresh</button>
      </div>
      <p className="text-zinc-500 text-sm mb-6">Teams waiting to be graded. Use the Judge tab to score them.</p>
      ...
      {loading ? (
        <div className="rounded-xl bg-zinc-900 border border-zinc-800 p-12 text-center text-zinc-500">Loading...</div>
      ) : arrivals.length === 0 ? (
        <div className="rounded-xl bg-zinc-900 border border-zinc-800 p-12 text-center text-zinc-600">No pending check-ins</div>
      ) : (
          ...
          {arrivals.map((a) => (
            <div key={a.checkInId} className="flex items-center justify-between rounded-xl bg-zinc-900 border border-zinc-800 px-5 py-4">
              ...
              <span className={a.arrivedAt
                ...
                {a.arrivedAt ? 'Checked in' : 'Waiting'}
              </span>
            </div>
          ))}
        </div>
