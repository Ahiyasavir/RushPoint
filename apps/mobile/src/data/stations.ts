...
import { routePathFor, fitRouteView, type GeoPoint, type RaceConfig } from '@rushpoint/shared';

/**
 * Builds a MapTiler Static Maps API URL: topographic "outdoor" style, framed at a
 * KNOWN center+zoom (via fitRouteView over ALL points) so a live GPS dot can be
 * projected on top accurately, with the route drawn as a line and a marker per
 * station. Rendered in an <Image>, so it works on web + native with no GL dep.
 *
 * Geography is now dynamic (raceConfig + stations from Firestore — see
 * useRaceConfig). Pass the SAME width/height here and to projectToPixel, and the
 * SAME `framePoints` to fitRouteView, so the overlay dot lines up with the image.
 *
...
export function buildStaticMapUrl(
  ...
  height: number,
  config: RaceConfig,
  stations: readonly GeoPoint[],
): string {
  const routePts = routePathFor(config);
  const v = fitRouteView(width, height, framePoints(config, stations));
  ...
  const pathCoords = routePts.map((p) => `${p.lng},${p.lat}`).join('|');
  const path = `stroke:${stroke}|width:4|fill:none|${pathCoords}`;
  const markerPts = [config.start, config.finish, config.gate, ...stations];
  const markers = markerPts.map((s) => `${s.lng},${s.lat}`).join('|');
